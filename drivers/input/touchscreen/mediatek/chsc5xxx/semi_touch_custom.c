...
static ssize_t cts_touch_glove_show(struct device *dev, struct device_attribute *attr, char *buf)
{
  ...
  //  count = sprintf(buf, "Glove Mode: %s, 0x8095 = %x\n", semi_touch_glove_switch(1)? "On" : "Off");
	count = snprintf(buf, PAGE_SIZE, "glove switch is %d, status is %d.\n",
...
int semi_is_gesture_wakeup_enabled(void)
{
	if ( ((IS_ERR(&st_dev)) ? 1 : 0)) {
		kernel_log_d("Failed to allocate ts memory, st_dev is NULL.\n");
		return 0;
	}

    return is_guesture_activate(st_dev.stc.ctp_run_status);
}
EXPORT_SYMBOL_GPL(semi_is_gesture_wakeup_enabled);

static ssize_t semi_gesture_show(
    struct device *dev, struct device_attribute *attr, char *buf)
{
    int count = 0;

    count = snprintf(buf, PAGE_SIZE, "guesture switch is %d, status is %d.\n",
...
static ssize_t semi_gesture_store(
    struct device *dev,
    struct device_attribute *attr, const char *buf, size_t count)
{
    ...
    if('0' == buf[0]){
		...
		kernel_log_d("semi gesture close\n");
    }else if('1' == buf[0]){
        open_guesture_function(st_dev.stc.custom_function_en);
		kernel_log_d("semi gesture open\n");
    }
...
static DEVICE_ATTR(gesture, S_IRUGO | S_IWUSR, semi_gesture_show,
                   semi_gesture_store);

int semi_gesture_sysfs_add(struct platform_device *pdev)
{
	int err = 0;
    kernel_log_d("Add device attr groups, semi_gesture_sysfs_add\n");

	err = device_create_file(&pdev->dev, &dev_attr_gesture);
	if (err) {
        kernel_log_d("sys file creation failed\n");
        return -ENODEV;
	}
	return 0;
}
...
{
	platform_set_drvdata(pdev, &st_dev);
	semi_gesture_sysfs_add(pdev);
